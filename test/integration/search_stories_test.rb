...
class SearchStoriesTest < ActionDispatch::IntegrationTest
  ...
  test "search for snop text" do
    ...
    # TODO: This test is the only one that doesn't work for some reason.
    # I suspect the reason is that the sunspot server has not indexed the title properly.
    # I tried running the sunspot server in test environment, but I was then unable to run
    # integration tests (permission denied: test.db).
    # I also tried to force an index but that made no difference either (Look at seeds.rb)
    # Seems like searching with sunspot in integration tests is a difficult thing, we will
    # have to look at this in more detail.
