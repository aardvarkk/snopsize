...
The devise views are located under link:snopsize/app/views/devise and can be skinned for custom UI.
...
If the URL of a article is <pre>www.theglobeandmail.com/articles/2349032</pre>

The domain is the part up to the first "/". In this case the domain would be:
<pre>www.theglobeandmail.com/</pre>

The resource is thus:
<pre>/articles/2349032</pre>
...
=== Domain Page
The domain page (link:snopsize/app/views/domains/show.html.erb) shows a table of resources for that domain. This allows the user to view all articles that have been snopped about for a particular domain (ex. www.theconomist.com/)
...
This means that each of: Home Page, Resource Page, Domain Page, Search Page, and User Page each have their own datatable (link:app/datatables). This defines a JSON output for the datatable that's show on each page. The tables are constructed in the respective Javascript files for each page (link:snopsize/app/assets/javascripts).
...
We are using jQuery to animate the browse view when the user is searching through the snops in the browse view.

=== Use of AJAX

== Testing

Unit Tests (link:snopsize/test/unit)
...
Functional Tests (link:snopsize/test/functional)
...
Integration Tests (link:snopsize/test/functional)
